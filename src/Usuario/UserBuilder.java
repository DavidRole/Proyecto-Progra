...
public class UserBuilder implements AbstractBuilder {
    ...
    public User getUser() throws UserException {
        ...
        if (usuario.getPassword() == null || usuario.getPassword().equals("")) {
            text += "La contraseña debe tener al menos una letra mayúscula (A – Z), tener al menos una letra minúscula (a – z),\n" + " incluir números (0 - 9), "
        ...
        if (usuario.getLastName() == null||usuario.getLastName().equals("") ) {
            text += "El apellido no debe contener números ni caracteres especiales y su largo no debe exceder los 30 caracteres.\n";
        }
        if (usuario.getPhoneNumber() == null || usuario.getPhoneNumber().equals("")) {
